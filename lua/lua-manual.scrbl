...
@defproc[(~lua-bytes [v lua-value/c]) bytes?]{
  Returns the result of calling the lua @tt{tostring} procedure on
  @racket[v].
}

@defproc[(~lua [v lua-value/c]) string?]{
  Like @racket[~lua-bytes] but converts the result to a string,
  decoding the bytes as UTF-8 and replacing any invalid sequences with
  @racket[#\uFFFD].
}
